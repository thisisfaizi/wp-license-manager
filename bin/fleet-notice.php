<?php
/**
 * Sign a fleet extension notice from the offline key backup — no WordPress needed.
 *
 * Usage:
 *   php bin/fleet-notice.php --pid=<profile code> --until=YYYY-MM-DD --keypair-file=/path/to/keypair.json
 ...
 * extend check-in by at most 30 days from now. --pid is the licence profile code of the product (the
 * product add-on's runbook gives it). Prints the notice on stdout; publish it at the product's fixed
 * static URL.
 *
...
$options = getopt( '', array( 'pid:', 'until:', 'keypair-file:', 'timezone::', 'now::' ) );

try {
	if ( ! extension_loaded( 'sodium' ) ) {
		throw new RuntimeException( 'The sodium PHP extension is required.' );
	}
	if ( empty( $options['pid'] ) || empty( $options['until'] ) || empty( $options['keypair-file'] ) ) {
		throw new InvalidArgumentException( 'Usage: php bin/fleet-notice.php --pid=<profile code> --until=YYYY-MM-DD --keypair-file=/path/to/keypair.json [--timezone=Asia/Karachi]' );
	}
	...
	$notice  = FleetNotice::build( $until, $now, (string) $options['pid'] );

	fwrite( STDERR, sprintf( "Fleet notice for %s: check-in may stretch to %s UTC (%.1f days).\n", $notice['pid'], gmdate( 'Y-m-d H:i:s', $until ), ( $until - $now ) / 86400 ) );
